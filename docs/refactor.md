...
当前编译器、模组与契约包版本为 0.11.0，source/catalog 修订为 11，包含候选匹配、合成网格、魔法配方、参数化整体构建和原生模型记录。格式与修订作为 schema 字面量由 Rust 常量生成，并通过契约包的 formats 导出；Web 不维护另一份版本判断。旧修订产物直接拒绝，需要使用对应新模组重新采集和编译。业务 API、类型、文件和方法不带演进版本号。
...
Research.itemTriggers保存Clue对象，字段为registry/meta/nbt/ore/matches。原始模板与NEI匹配示例分离，没有示例也保留完整声明；minecraft:portal无需伪造items行。matches有序去重并校验物品引用，反向links只收录示例。NBT保留类型和long精度；编译器验证结构与引用，不重演任意游戏API匹配。旧字符串数组退出当前契约。
# Native member analysis (revision 13)

NESQL 0.14.0 and Compiler/contracts 0.13.0 use source/catalog revision 13. Revision 12 snapshots are retained, but must not be read with this contract; export and compile a new snapshot. This is a contract change, not a migration reader.

The scanner's Forestry NEI outputs are labels without genomes. They are not fixed products. A `member` input rule names `rootTrees`, `rootBees` or `rootButterflies` and an `analyzed` state. It matches valid native members of the example's item registry and the stated root/state, including other genomes. Analysis state follows the pinned Forestry reader (`IsAnalyzed`, Minecraft boolean conversion, absent means false); malformed genomes are not promised valid. Metadata is retained by the native operation rather than replaced by the example.

`consume.kind = stack` consumes the whole offered stack; choice `amount = "1"` is a minimum and an example, not a per-item charge. An output `change.action.kind = analyze` runs the Forestry 4.10.17 member reader on a copy. If `analyze()` changes the individual, `writeToNBT` writes a fresh compound into a copy of the input stack. Otherwise the original tags are copied unchanged. Both branches preserve item, metadata and offered stack size. The unanalyzed branch costs 500 base ticks, 2 EU/t and 100 mB of the native honey fluid. The already analyzed branch costs 1 tick and 1 EU/t; 100 mB honey must be present but is kept. These are separate recipes and no costs are inferred from the NEI placeholder.
