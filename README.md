...
Research itemTriggers contain Clue objects with raw registry, meta, typed nbt, primary ore group and sorted matches item IDs. A minecraft:portal pattern with metadata 32767 can have no examples; it remains a declared condition. Only matches require item records and generate reverse links. Validation checks pattern structure, NBT, budgets and references, without inventing examples or reproducing arbitrary game hooks. Revision 10 string trigger arrays are rejected.

Constructed block metadata preserves the unsigned 16-bit range (0–65535) used by GTNH 2.8.4 and NotEnoughIDs. Values above 15, including GT frame material IDs, are kept in identities and catalogs without truncation. Older compiler binaries reject those valid states; use compiler 0.11.1 or later for NESQL 0.12.2 captures. The record shape and revision are unchanged.
